...
\ifans{
    \begin{enumerate}[label=\roman*.]
        ...
        \[
        \bJ_{\text{naive-softmax}}(\bv_c, o, \bU) = -\sum_{w \in \text{Vocab}} \by_w \log(\hat{\by}_w) = - \log( \hat{\by}_o ).
        \]
        Also, we know that
        \[
        \hat{\by}_o = P(O=o \mid C=c) = \frac{\exp(\bu_{o}^\top \bv_c)}{\sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)}.
        \]
        Therefore,
        \[
        \begin{aligned}
            \pd{\bJ_{\text{naive-softmax}}}{\bv_c} = \frac{1}{\hat{\by}_o} \left( \sum_x \frac{\exp(\bu_x^\top \bv_c)}{\sum_{w} \exp(\bu_w^\top \bv_c)} \bu_x \right) = \frac{1}{\by^\top \hat{\by}} \bU (\hat{\by} - \by).
        \end{aligned}
        \]

        \item The gradient is zero when $\hat \by = \by$.
        \item The term $\bU \hat \by$ is the expected outside word for $c$, and $\bU \by$ is the actual outside word for $c$.
    \end{enumerate}
}
