...
\ifans{
    Since the true distributino $\by$ is a one-hot vector on entry $o$, the sum is zero for all terms but one. Therefore, we have
    \[
    -\sum_{w \in \text{Vocab}} \by_w \log(\hat{\by}_w) = - \log (\hat{\by}_o),
    \]
    as desired.
...
\ifans{
    \begin{enumerate}[label=\roman*.]
        \item As proved in (a), the naive-softmax loss
        \[
        \bJ_{\text{naive-softmax}}(\bv_c, o, \bU) = -\sum_{w} \by_w \log(\hat{\by}_w) = - \log( \hat{\by}_o ).
        \]
        Also, we know that
        \[
        \hat{\by}_o = P(O=o \mid C=c) = \frac{\exp(\bu_{o}^\top \bv_c)}{\sum_{w} \exp(\bu_{w}^\top \bv_c)}.
        \]
        Therefore,
        \[
        \begin{aligned}
            \pd{\bJ_{\text{naive-softmax}}}{\bv_c} = - \bu_o + \sum_x \frac{\exp(\bu_x^\top \bv_c)}{\sum_{w} \exp(\bu_w^\top \bv_c)} \bu_x = \bU (\hat{\by} - \by).
        \end{aligned}
        \]

        \item The gradient is zero when $\hat \by = \by$.
        \item The term $\bU \hat \by$ is the expected outside word for $c$, and $\bU \by$ is the actual outside word for $c$. When the gradient is subtracted from the word vector $v_c$, the difference between the expected outside word and the actual outside word decreases.
    \end{enumerate}
}
...
\ifans{
    When $\mathbf{u}_x = \alpha\mathbf{u}_y$ for some words $x \neq y$ and some positive scalar $\alpha$, their normalized value will be the same vector. By using L2 normalized vectors, the sum of individual embeddings changes, including its length and direction. When the length and direction matters, for example, when we classify based on the direction of the embedding sum, L2 normalization takes away useful information.
}
...
\ifans{
    \[
    \pd{\bJ_{\text{naive-softmax}}}{\bu_w} = - 1\{w = o\} \bv_c + \frac{\exp(\bu_w^\top \bv_c)}{\sum_{x} \exp(\bu_x^\top \bv_c)} \bv_c = - 1\{w = o\} \bv_c + \hat{\by}_w \bv_c.
    \]
}
...
\ifans{
    We have
    \[
    \pd{\bJ_{\text{naive-softmax}}}{\bu_w} = - 1\{w = o\} \bv_c + \hat{\by}_w \bv_c,
    \]
    which makes the columns of the matrix. Therefore
    \[
    \pd{\bJ_{\text{naive-softmax}}}{\bU} = \bv_c (\hat \by - \by)^\top.
    \]
}
